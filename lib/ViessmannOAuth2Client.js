...
module.exports = class ViessmannOAuth2Client extends OAuth2Client {
  ...
  // Required:
  static API_URL = 'https://api.viessmann-climatesolutions.com/iot/v2';
  static TOKEN_URL = 'https://iam.viessmann-climatesolutions.com/idp/v3/token';
  static AUTHORIZATION_URL = 'https://iam.viessmann-climatesolutions.com/idp/v3/authorize';
  ...
  /*
   * Pull the human-readable part out of a Viessmann error body. The API
   * reports failures as
   *   { statusCode, errorType, message, extendedPayload: { reason, details } }
   * where `details` carries the actually useful text, e.g.
   *   Value 'eco' is not within allowed values: efficientWithMinComfort, …
   * There is no `body.error` field on this API, which is why every 4xx used
   * to surface as "Unknown error".
   */
  static describeApiError(body) {
    if (!body || typeof body !== 'object') return null;
    if (body.extendedPayload && body.extendedPayload.details) return body.extendedPayload.details;
  ...
  async onHandleNotOK({ body, status }) {
    ...
    const detail = this.constructor.describeApiError(body);
    switch (status) {
      case 401:
      ...
      case 429:
        throw new OAuth2Error(detail ? `Rate limit exceeded: ${detail}` : 'Rate limit exceeded');
      default:
        throw new OAuth2Error(detail || `Request failed with status ${status}`);
    }
  }
